...
(function () {
    ...
    function isAcceptButton(el) {
        ...
        const patterns = ['accept', 'run', 'retry', 'apply', 'execute', 'confirm', 'allow once', 'always allow', 'allow'];
        const rejects = ['skip', 'reject', 'cancel', 'close', 'refine', 'always run'];
        if (rejects.some(r => text.includes(r))) return false;
        if (!patterns.some(p => text.includes(p))) return false;

        // Skip dropdown/listbox trigger buttons (e.g. HeadlessUI "Always run" dropdown)
        if (el.getAttribute('aria-haspopup') === 'listbox' || el.getAttribute('role') === 'listbox') return false;
        if (el.id && el.id.includes('headlessui-listbox')) return false;
        // Also skip if a parent is a listbox button (text might come from a child span)
        const closestListbox = el.closest('[aria-haspopup="listbox"], [id*="headlessui-listbox"]');
        if (closestListbox) return false;
